...
//  battleship
//
//  Created by Nick Kai on 9/26/17.
//  Copyright © 2017 Nick Kai. All rights reserved.
//
#include <iostream>
#include <string>
using namespace std;
...
int main()
{
    ...
    for (int aplyr=1; aplyr<3; ++aplyr)
    {
        ...
        for (int thisShip=0; thisShip<1/*SHIP_TYPES*/; ++thisShip)
        {
...
void DrawBoard(int thisPlayer)
{
    ...
    for (int h=0; h<BOARD_HEIGHT; ++h){
   // for (char h = 'A'; h < 'A' + BOARD_HEIGHT; ++h) {
        for (int w=0; w<BOARD_WIDTH; ++w){
